...
function setupResultsSection() {
  ...
  if (!resultClassSelect || !resultStudentSelect || !resultSubjectContainer || !resultForm) {
  ...
  resultClassSelect.addEventListener('change', async () => {
    const selectedClass = resultClassSelect.value;
    ...
    try {
      const studentRes = await fetch(`${API_URL}/api/students?class=${encodeURIComponent(selectedClass)}`);
      const students = await studentRes.json();

      students.forEach(s => {
        const option = document.createElement('option');
        option.value = s._id || s.id;
  ...
  resultStudentSelect.addEventListener('change', async () => {
    ...
    resultSubjectContainer.innerHTML = 'Loading subjects...';

    try {
      const subjectRes = await fetch(`${API_URL}/api/classes/${encodeURIComponent(className)}/subjects`);
      const subjects = await subjectRes.json();

      resultSubjectContainer.innerHTML = '';
      subjects.forEach(subject => {
        const div = document.createElement('div');
        div.classList.add('subject-entry', 'mb-2');
        div.innerHTML = `
          <h3>${subject.name}</h3>
          <label>Test 1: <input class="score-input" data-subject="${subject.name}" data-type="test1" type="number" /></label>
          <label>Test 2: <input class="score-input" data-subject="${subject.name}" data-type="test2" type="number" /></label>
          <label>Exam: <input class="score-input" data-subject="${subject.name}" data-type="exam" type="number" /></label>
          <label>Total: <input class="score-input" data-subject="${subject.name}" data-type="total" type="number" readonly /></label>
          <label>Grade: <input class="score-input" data-subject="${subject.name}" data-type="grade" type="text" readonly /></label>
        `;
        resultSubjectContainer.appendChild(div);
      });
    } catch (err) {
  ...
  resultSubjectContainer.addEventListener('input', e => {
    if (!e.target.classList.contains('score-input')) return;

    const subjectDiv = e.target.closest('.subject-entry');
    const test1 = Number(subjectDiv.querySelector('[data-type="test1"]').value) || 0;
    const test2 = Number(subjectDiv.querySelector('[data-type="test2"]').value) || 0;
    const exam = Number(subjectDiv.querySelector('[data-type="exam"]').value) || 0;

    const totalInput = subjectDiv.querySelector('[data-type="total"]');
    const gradeInput = subjectDiv.querySelector('[data-type="grade"]');
...
document.querySelectorAll('.score-input').forEach(input => {
  ...
  // Convert numeric fields to numbers, leave grade as string
  results[subject][type] = type === 'grade' ? input.value : Number(input.value) || 0;
});
    ...
    try {
      const res = await fetch(`${API_URL}/api/results`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId, className, results })
      });
